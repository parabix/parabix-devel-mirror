...
LLVM_Gen_RetVal compile(bool show_compile_time, Encoding encoding, std::string input_string)
{
    ...
    CodeGenState cg(symgen);

    CC_Compiler cc_compiler(cg, encoding);
    cc_compiler.compile(re_map);
    #ifdef DEBUG_PRINT_PBIX_AST
    //Print to the terminal the AST that was generated by the character class compiler.
    std::cerr << "Pablo CC AST:" << std::endl << StatementPrinter::Print_CC_PabloStmts(cg.expressions()) << std::endl;
    //Print to the terminal the AST that was generated for the re subexpressions.
    std::cerr << "Subexpressions:" << StatementPrinter::PrintStmts(cg) << std::endl;
    #endif

    RE_Compiler pbix_compiler(cg, name_map);
    pbix_compiler.compile(re_ast);
    #ifdef DEBUG_PRINT_PBIX_AST
    //Print to the terminal the AST that was generated by the pararallel bit-stream compiler.
    std::cerr << "Final Pablo AST:" << StatementPrinter::PrintStmts(cg) << ")" << std::endl;
    ...
    LLVM_Gen_RetVal retVal = irgen.Generate_LLVMIR(cg);
    if (show_compile_time)
    {

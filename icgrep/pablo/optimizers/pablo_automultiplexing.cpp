...
void AutoMultiplexing::generateVariableConstraints(const PabloBlock & block, const std::vector<pablo::Var *> & inputVars) {

    Engine engine(1000, inputVars.size());
    std::unordered_map<const PabloAST *, BDD> transitoryNodes;

    for (auto i = 0; i != inputVars.size(); ++i) {
        transitoryNodes.insert(std::make_pair(inputVars[i], engine.var(i)));
    }
    ...
    while(true) {
        for (; stmt; stmt = stmt->getNextNode()) {
            ...
            if (isa<If>(stmt) || isa<While>(stmt)) {
                ...
                PabloBlock & b = isa<If>(stmt) ? cast<If>(stmt)->getBody() : cast<While>(stmt)->getBody();
            ...
            if (isa<Call>(stmt)) {
                // TODO: Have a BDD precomputed for each called function?
                // Note: this assumes that no Call occurs twice in the function.
                bdd = engine.addVar();
            }
            else {
                ...
                for (unsigned i = 0; i != stmt->getNumOperands(); ++i) {
                    auto f = transitoryNodes.find(stmt->getOperand(i));
                    if (f == transitoryNodes.end()) {
                        ignore = true;
                        break;
                    }
                    nodes[i] = f->second;
                ...
                switch (stmt->getClassTypeId()) {
                    case PabloAST::ClassTypeId::And:
                        bdd = engine.applyAND(nodes[0], nodes[1]);
                        break;
                    case PabloAST::ClassTypeId::Or:
                        bdd = engine.applyOR(nodes[0], nodes[1]);
                        break;
                    case PabloAST::ClassTypeId::Xor:
                        bdd = engine.applyXOR(nodes[0], nodes[1]);
                        break;
                    case PabloAST::ClassTypeId::Not:
                        bdd = engine.applyNOT(nodes[0]);
                        break;
                    case PabloAST::ClassTypeId::Sel:
                        bdd = engine.applySEL(nodes[0], nodes[1], nodes[2]);
                        break;
                    default:
                        ignore = true;
                }
            ...
            transitoryNodes.insert(std::make_pair(stmt, bdd));
        }
        if (scope.empty()) {
            break;
        }
...
//        const Instruction * const instI = variables[i];
//        _variableConstraints.add(instI);
//        #ifdef ALLOW_SUBSET_CONSTRAINT_TRANSFORMATIONS
//        _subsetConstraints.add(instI);
//        #endif
//        const bdd & A = transitoryVariables.find(instI)->second;

//        // std::cout << bddset << A << std::endl;

//        for (unsigned j = 0; j < i; ++j) {
//            const Instruction * const instJ = variables[j];
//            // are these variables within differing basic blocks?
//            bool constrained = true;
//            // if (instI->getParent() == instJ->getParent()) {
//                // or is there any truth assignment can satisfy both "character class" equations?
//                const bdd & B = transitoryVariables.find(instJ)->second;
//                const bdd C = A & B;
//                // is there any satisfying truth assignment?
//                if (bdd_satone(C) == bdd_false()) {
//                    constrained = false;
//                }
//                #ifdef ALLOW_SUBSET_CONSTRAINT_TRANSFORMATIONS
//                // is one a subset of the other?
//                else if (A == C) {
//                    _subsetConstraints.addConstraint(instI, instJ);
//                    constrained = false;
//                }
//                else if (B == C) {
//                    _subsetConstraints.addConstraint(instJ, instI);
//                    constrained = false;
//                }
//                #endif
//            // }
//            if (constrained) {
//                // if so, these these character classes are not mutually exclusive and cannot be
//                // multiplexed together; mark the constraint accordingly.
//                _variableConstraints.addConstraint(instI, instJ);
//            }
//        }

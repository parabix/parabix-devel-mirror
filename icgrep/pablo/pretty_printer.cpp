...
static void PrintStatement(Statement const * stmt, raw_ostream & out, const bool expandNested, unsigned indent) noexcept {
    ...
    if (stmt == nullptr) {
        ...
        if (expandNested) {
            out << " {\n";
            PabloPrettyPrinter::print(br->getBody(), out, expandNested, indent + INDENT_WIDTH);
            out.indent(indent) << "}\n";
        } else {
            out << " {...}\n";
        }
...
void PabloPrettyPrinter::print(PabloKernel const * kernel, raw_ostream & out) noexcept {
...
void PabloPrettyPrinter::print(PabloAST const * node, raw_ostream & out) noexcept {
    PrintExpression(node, out);
}

void PabloPrettyPrinter::print(PabloBlock const * block, raw_ostream & out, const bool expandNested, unsigned indent) noexcept {
...
void PabloPrettyPrinter::print(Statement const * stmt, raw_ostream & out, const bool expandNested, unsigned indent) noexcept {
    PrintStatement(stmt, out, expandNested, indent);
}

...
void caseInsensitiveInsertRange(re::CC * base_cc, codepoint_t lo, codepoint_t hi) {
    base_cc->insert_range(lo, hi);
    ...
    while (foldTable[e].range_lo <= hi) {
      ...
      if (fe.fold_offset > 0 && fe.range_lo + fe.fold_offset < fnext.range_lo) {
        ...
        base_cc -> insert_range(lo1 + fe.fold_offset, hi1 + fe.fold_offset);
      }
      ...
      for (int i = 0; i < fe.fold_pairs.size(); i++) {
        ...
        base_cc->insert(fe.fold_pairs[i].second);
      }

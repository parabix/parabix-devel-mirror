...
Value * bitblock_advance_ci_co(IDISA::IDISA_Builder * iBuilder, Value * val, unsigned shift, Value * stideCarryArr, unsigned carryIdx){   
...
void editdGPUKernel::generateDoBlockMethod() {
    ...
    for(unsigned i = 1; i<mPatternLen; i++){     
        ...
        Value * adv = bitblock_advance_ci_co(iBuilder, e[i-1][0], 1, stideCarryArr, carryIdx++);
        e[i][0] = iBuilder->CreateAnd(adv, pattStream);
        for(unsigned j = 1; j<= mEditDistance; j++){
            Value * adv0 = bitblock_advance_ci_co(iBuilder, e[i-1][j], 1, stideCarryArr, carryIdx++);
            Value * adv1 = bitblock_advance_ci_co(iBuilder, e[i-1][j-1], 1, stideCarryArr, carryIdx++);
            Value * adv2 = bitblock_advance_ci_co(iBuilder, e[i][j-1], 1, stideCarryArr, carryIdx++);

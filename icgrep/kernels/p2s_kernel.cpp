...
inline void p2s(IDISA::IDISA_Builder * iBuilder, Value * p[], Value * output) {
    Value * bit00004444[2];
    Value * bit22226666[2];
    Value * bit11115555[2];
    Value * bit33337777[2];
    ...
    for (unsigned j = 0; j < 8; ++j) {
        iBuilder->CreateBlockAlignedStore(s[j], output, {iBuilder->getInt32(j)});
    }
}
    		
void generateP2SKernel(Module *, IDISA::IDISA_Builder * iBuilder, KernelBuilder * kBuilder) {
    ...
    Value * input[8];
    Value * output = kBuilder->getOutputStream(0);
    for (unsigned j = 0; j < 8; ++j) {

...
std::unique_ptr<llvm::Module> scanMatchKernel::createKernelModule() {
    std::unique_ptr<llvm::Module> theModule = KernelInterface::createKernelModule();
    
    Function * scanWordFunction = generateScanWordRoutine(theModule.get());
    ...
    Value * matchResults = getParameter(finalBlockFunction, "matchResults");
    iBuilder->CreateCall(doBlockFunction, {self, matchResults});
    iBuilder->CreateRetVoid();
    //
    return theModule;
}

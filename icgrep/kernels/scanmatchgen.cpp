...
void MatchCoordinatesKernel::generateMultiBlockLogic(const std::unique_ptr<KernelBuilder> & b, Value * const numOfStrides) {
    ...
    //->CallPrintInt("stridePos", stridePos);
    ...
    pendingLineStart->addIncoming(strideFinalLineNumPhi, strideCoordinatesDone);
    if (mLineNumbering) {
        pendingLineNum->addIncoming(strideFinalLineNumPhi, strideCoordinatesDone);
    }
//    b->CallPrintInt("strideNo", strideNo);
//    b->CallPrintInt("finalStrideMatchCount", finalStrideMatchCount);
//    b->CallPrintInt("strideFinalLineStart", strideFinalLineStart);
...
void MatchReporter::generateDoSegmentMethod(const std::unique_ptr<KernelBuilder> & b) {
    ...
    Value * nextMatchNum = b->CreateAdd(phiMatchNum, ONE);

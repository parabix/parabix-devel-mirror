
#ifndef ICGREP_LZ4_PARALLEL_BYTESTREAM_AIO_H
#define ICGREP_LZ4_PARALLEL_BYTESTREAM_AIO_H

#include "kernels/kernel.h"
#include <string>
#include <map>
#include <vector>
#include <llvm/IR/DerivedTypes.h>

namespace llvm {
    class Module;
    class Function;
    class BasicBlock;
    class Value;
}

namespace IDISA { class IDISA_Builder; }


namespace kernel {

    class LZ4ParallelByteStreamAioKernel : public SegmentOrientedKernel {

    public:
        // By default, output block size in LZ4 is 4MB
        LZ4ParallelByteStreamAioKernel(const std::unique_ptr<kernel::KernelBuilder> &b, unsigned lz4BlockSize, bool enableGather = true, bool enableScatter = true, int minParallelLevel = 1);

    protected:
        void generateDoSegmentMethod(const std::unique_ptr<KernelBuilder> &b) override;

    private:


        // ---- SIMD
        void generateSimdDecompression(const std::unique_ptr<KernelBuilder> &iBuilder, llvm::Value* blockDataIndex);
        void generateSimdDecompression1(const std::unique_ptr<KernelBuilder> &iBuilder, llvm::Value* blockDataIndex);
        void generateSimdDecompression2(const std::unique_ptr<KernelBuilder> &iBuilder, llvm::Value* blockDataIndex);
        std::pair<llvm::Value*, llvm::Value*> simdProcessBlockBoundary(const std::unique_ptr<KernelBuilder> &b,
                                                                       llvm::Value *tokenPosVec, llvm::Value *lz4BlockEnd, llvm::Value* outputPosVec);
        std::pair<llvm::Value*, llvm::Value*> simdProcessBlockBoundaryByLoop(const std::unique_ptr<KernelBuilder> &b,
                                                                       llvm::Value *tokenPosVec, llvm::Value *lz4BlockEnd, llvm::Value* outputPosVec);
        llvm::Value* generateSimdAcceleration(const std::unique_ptr<KernelBuilder> &b, llvm::Value *beginTokenPosVec,
                                              llvm::Value *blockEndVec); // TODO incomplete
        // Literal
        std::pair<llvm::Value*, llvm::Value*> parallelParseLiteralInfo(const std::unique_ptr<KernelBuilder> &b, llvm::Value *tokenPosVec, llvm::Value* tokenValuesVec, llvm::Value* notFinishMask);
        std::pair<llvm::Value*, llvm::Value*> simdParseLiteralInfo(const std::unique_ptr<KernelBuilder> &b, llvm::Value *tokenPosVec, llvm::Value* tokenValuesVec, llvm::Value* notFinishMask);
        std::pair<llvm::Value*, llvm::Value*> simdParseLiteralInfo2(const std::unique_ptr<KernelBuilder> &b, llvm::Value *tokenPosVec, llvm::Value* tokenValuesVec, llvm::Value* notFinishMask);
        std::pair<llvm::Value*, llvm::Value*> parseMultipleLiteralInfoByLoop(const std::unique_ptr<KernelBuilder> &b, llvm::Value *tokenPosVec, llvm::Value* tokenValuesVec, llvm::Value* notFinishMask);

        // Match
        std::pair<llvm::Value*, llvm::Value*> parallelParseMatchInfo(const std::unique_ptr<KernelBuilder> &b, llvm::Value *matchOffsetPosVec, llvm::Value* tokenValuesVec, llvm::Value* notFinishMask);
        std::pair<llvm::Value*, llvm::Value*> simdParseMatchInfo(const std::unique_ptr<KernelBuilder> &b, llvm::Value *matchOffsetPosVec, llvm::Value* tokenValuesVec, llvm::Value* notFinishMask);
        std::pair<llvm::Value*, llvm::Value*> simdParseMatchInfo2(const std::unique_ptr<KernelBuilder> &b, llvm::Value *matchOffsetPosVec, llvm::Value* tokenValuesVec, llvm::Value* notFinishMask);
        std::pair<llvm::Value*, llvm::Value*> parseMultipleMatchInfoByLoop(const std::unique_ptr<KernelBuilder> &b, llvm::Value *matchOffsetPosVec, llvm::Value* tokenValuesVec, llvm::Value* notFinishMask);


        // Copy
        void handleSimdLiteralCopy(const std::unique_ptr<KernelBuilder> &b, llvm::Value* literalStartVec, llvm::Value* literalLengthVec, llvm::Value* outputPosVec);
        void generateSimdSequentialLiteralCopy(const std::unique_ptr<KernelBuilder> &b, llvm::Value *literalStartVec,
                                               llvm::Value *literalLengthVec, llvm::Value *outputPosVec);
        void generateSimdLiteralCopyByScatter(const std::unique_ptr<KernelBuilder> &b, llvm::Value *literalStartVec,
                                              llvm::Value *literalLengthVec, llvm::Value *outputPosVec);
        void handleSimdMatchCopy(const std::unique_ptr<KernelBuilder> &b, llvm::Value* matchOffsetVec, llvm::Value* matchLengthVec, llvm::Value* outputPosVec);
        void generateSimdSequentialMatchCopy(const std::unique_ptr<KernelBuilder> &b, llvm::Value* matchOffsetVec, llvm::Value* matchLengthVec, llvm::Value* outputPosVec);
        void generateSimdMatchCopyByScatter(const std::unique_ptr<KernelBuilder> &b, llvm::Value* matchOffsetVec, llvm::Value* matchLengthVec, llvm::Value* outputPosVec);


        // ---- Sequential
        void generateSequentialDecompression(const std::unique_ptr<KernelBuilder> &b, llvm::Value* startBlockDataIndex, llvm::Value* endBlockDataIndex);
        llvm::Value* generateProcessCompressedBlock(const std::unique_ptr<KernelBuilder> &b, llvm::Value *lz4BlockStart,
                                                    llvm::Value *lz4BlockEnd, llvm::Value* initOutputPos);
        std::pair<llvm::Value*, llvm::Value*> processBlockBoundary(const std::unique_ptr<KernelBuilder> &b,
                                                                   llvm::Value *beginTokenPos, llvm::Value *lz4BlockEnd, llvm::Value* outputPos);
        std::pair<llvm::Value *, llvm::Value *> parseLiteralInfo(const std::unique_ptr<KernelBuilder> &b, llvm::Value *tokenPos, llvm::Value* tokenValue);
        std::pair<llvm::Value *, llvm::Value *> parseMatchInfo(const std::unique_ptr<KernelBuilder> &b, llvm::Value *matchOffsetPos, llvm::Value* tokenValue);

        // Copy
        void handleLiteralCopy(const std::unique_ptr<KernelBuilder> &b, llvm::Value* literalStart, llvm::Value* literalLength, llvm::Value* outputPos);
        void handleMatchCopy(const std::unique_ptr<KernelBuilder> &b, llvm::Value* matchOffset, llvm::Value* matchLength, llvm::Value* outputPos);


        // ---- Basic Method
        // -- SIMD
        // Load
        llvm::Value *
        generateLoadSimdInt64NumberInput(const std::unique_ptr<KernelBuilder> &iBuilder, std::string inputBufferName,
                                         llvm::Value *globalOffset);
        llvm::Value* simdFetchData(const std::unique_ptr<KernelBuilder> &b, llvm::Value* basePtr, llvm::Value* offsetVec, llvm::Value* mask);
        llvm::Value* simdFetchByteData(const std::unique_ptr<KernelBuilder> &b, llvm::Value* basePtr, llvm::Value* offsetVec, llvm::Value* mask);
        llvm::Value* simdFetchI64DataByGather(const std::unique_ptr<KernelBuilder> &b, llvm::Value *basePtr,
                                              llvm::Value *offsetVec, llvm::Value *mask);
        llvm::Value* simdFetchI32DataByGather(const std::unique_ptr<KernelBuilder> &b, llvm::Value *basePtr,
                                              llvm::Value *offsetVec, llvm::Value *mask);
        llvm::Value* simdFetchDataByLoop(const std::unique_ptr<KernelBuilder> &b, llvm::Value *basePtr,
                                         llvm::Value *offsetVec, llvm::Value *mask, unsigned resultBitWidth = 64);
        // Store
        void simdPutData(const std::unique_ptr<KernelBuilder> &b, llvm::Value* basePtr, llvm::Value* offsetVec,llvm::Value* values, llvm::Value* mask /*i256*/);
        void simdPutDataByLoop(const std::unique_ptr<KernelBuilder> &b, llvm::Value* basePtr, llvm::Value* offsetVec,llvm::Value* values, llvm::Value* mask /*i256*/);
        void simdPutDataByScatter(const std::unique_ptr<KernelBuilder> &b, llvm::Value* basePtr, llvm::Value* offsetVec,llvm::Value* values, llvm::Value* mask /*i256*/);

        // Memcpy


        // -- Sequential
        // Load
        llvm::Value *
        generateLoadInt64NumberInput(const std::unique_ptr<KernelBuilder> &iBuilder, std::string inputBufferName,
                                     llvm::Value *globalOffset);
        // Memcpy
        void generateOverwritingMemcpy(const std::unique_ptr<KernelBuilder> &b, llvm::Value *inputBasePtr,
                                       llvm::Value *outputBasePtr, llvm::Value *copyBytes, llvm::PointerType *targetPtrTy,
                                       size_t stepSize);
        void generateOverwritingMemcpy(const std::unique_ptr<KernelBuilder> &b, llvm::Value *inputBasePtr,
                                       llvm::Value *outputBasePtr, llvm::Value *copyBytes, llvm::PointerType *targetPtrTy,
                                       llvm::Value* stepSize);


        // ---- Data Member
        size_t mMininumParallelLevel;
        bool mEnableGather;
        bool mEnableScatter;
        unsigned mLz4BlockSize;


        // ---- Parallel Level Measurement
        void initParallelLevelMeasurement(const std::unique_ptr<KernelBuilder> &b);
        void recordParallelLevel(const std::unique_ptr<KernelBuilder> &b, llvm::Value* v);
        void printParallelLevelResult(const std::unique_ptr<KernelBuilder> &b);


        // ---- Constants and Types
        void initializeConstantsAndTypes(const std::unique_ptr<KernelBuilder> &b);

        llvm::Value* BIT_BLOCK_0;
        llvm::Value* BIT_BLOCK_1;
        llvm::Value* BIT_BLOCK_F0;
        llvm::Value* BIT_BLOCK_0F;
        llvm::Value* BIT_BLOCK_FF;
        llvm::Value* BIT_BLOCK_FFFF;
        llvm::Constant* INT_BIT_BLOCK_TY_0;

        llvm::Constant* SIZE_0;
        llvm::Constant* SIZE_1;
        llvm::Constant* BYTE_FF;
        llvm::Constant* BYTE_F0;
        llvm::Constant* BYTE_0F;

        llvm::Type* INT_BIT_BLOCK_TY;
        llvm::Type* BIT_BLOCK_TY;
    };

}


#endif //ICGREP_LZ4_PARALLEL_BYTESTREAM_AIO_H

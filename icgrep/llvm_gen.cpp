...
LLVM_Gen_RetVal LLVM_Generator::Generate_LLVMIR(const CodeGenState & cg_state)
{
    ...
    SetReturnMarker(Generate_PabloStatements(cg_state.expressions()), 0);
...
Value* LLVM_Generator::GetMarker(const std::string & name)
{
    IRBuilder<> b(mBasicBlock);
...
void LLVM_Generator::SetReturnMarker(Value * marker, const unsigned output_idx)
{
    IRBuilder<> b(mBasicBlock);
...
Value * LLVM_Generator::Generate_PabloStatements(const ExpressionList & stmts) {
    Value * retVal = nullptr;
    for (PabloE * statement : stmts) {
        retVal = Generate_PabloS(statement);
    }
    return retVal;
}

Value * LLVM_Generator::Generate_PabloS(const PabloE *stmt)
{
    Value * retVal = nullptr;
    ...
    if (const Assign * assign = dyn_cast<const Assign>(stmt))
    {
        ...
        b.CreateStore(Generate_PabloE(assign->getExpr()), marker);
    ...
    else if (const If * ifstmt = dyn_cast<const If>(stmt))
    {
        ...
        Value *  returnMarker = Generate_PabloStatements(ifstmt->getPSList());
    ...
    else if (const While* whl = dyn_cast<const While>(stmt))
    {
        ...
        Value * returnMarker = Generate_PabloStatements(whl->getPSList());
...
Value* LLVM_Generator::Generate_PabloE(const PabloE * expr)
{
    Value* retVal = 0;
    IRBuilder<> b(mBasicBlock);
    ...
    else if (const Call* call = dyn_cast<Call>(expr))
    {
        ...
        if (f == mMarkerMap.end()) {
            ...
            f = mMarkerMap.insert(make_pair(call->getCallee(), ptr)).first;
        }
        retVal = b.CreateLoad(f->second);
    }
    else if (const Var * var = dyn_cast<Var>(expr))
    {
        retVal = b.CreateLoad(GetMarker(var->getName()), false, var->getName());
    }
    else if (const And * pablo_and = dyn_cast<And>(expr))
    {
        retVal = b.CreateAnd(Generate_PabloE(pablo_and->getExpr1()), Generate_PabloE(pablo_and->getExpr2()), "and");
    }
    else if (const Or * pablo_or = dyn_cast<Or>(expr))
    {
        retVal = b.CreateOr(Generate_PabloE(pablo_or->getExpr1()), Generate_PabloE(pablo_or->getExpr2()), "or");
    }
    ...
    {
        Value* marker_expr = Generate_PabloE(mstar->getExpr1());
        Value* cc_expr = Generate_PabloE(mstar->getExpr2());
    ...
    {
        Value* marker_expr = Generate_PabloE(sthru->getScanFrom());
        Value* cc_expr = Generate_PabloE(sthru->getScanThru());

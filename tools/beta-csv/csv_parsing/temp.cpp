...
public:
    ...
    StreamSet * translatedBasis, StreamSet * Dquote_surrounding, StreamSet * field_starts, StreamSet * field_follows, 
     StreamSet * record_starts, StreamSet * record_follows, StreamSet * mask, StreamSet * escapes)
...
void CSV_Masking::generatePabloMethod() {
    ...
    PabloAST * head = pb.createNot(pb.createAdvance(pb.createOnes(),1));
...
CSVTranslateFunctionType generatePipeline(CPUDriver & pxDriver) {
    ...
    Field_follows, Record_starts, Record_follows, CSV_data_mask, escapes);
    /*

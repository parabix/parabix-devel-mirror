...
void HexToBinary::generateDoBlockMethod(BuilderRef b) {
    ...
    #else
        Constant * splat_0 = ConstantVector::getSplat({bytesPerBlock,false}, ConstantInt::get(i8Ty, '0'));
        Constant * splat_9 = ConstantVector::getSplat({bytesPerBlock,false}, ConstantInt::get(i8Ty, '9'));
        Constant * case_shift = ConstantVector::getSplat({bytesPerBlock,false}, ConstantInt::get(i8Ty, 'a' - 'A'));
        Constant * splat_a = ConstantVector::getSplat({bytesPerBlock,false}, ConstantInt::get(i8Ty, 'a'));
        Constant * splat_f = ConstantVector::getSplat({bytesPerBlock,false}, ConstantInt::get(i8Ty, 'f'));
        Constant * af_cvt = ConstantVector::getSplat({bytesPerBlock,false}, ConstantInt::get(i8Ty, 'a' - 10));
    #endif
...
void BinaryToHex::generateDoBlockMethod(BuilderRef b) {
    ...
    #else
        Constant * splatCh_0 = ConstantVector::getSplat({bytesPerBlock, false}, ConstantInt::get(i8Ty, '0'));
        Constant * splat_9 = ConstantVector::getSplat({bytesPerBlock, false}, ConstantInt::get(i8Ty, 9));
        Constant * splatCh_a = ConstantVector::getSplat({bytesPerBlock, false}, ConstantInt::get(i8Ty, 'a'-10));
    #endif
    for (unsigned i = 0; i < 4; i++) {

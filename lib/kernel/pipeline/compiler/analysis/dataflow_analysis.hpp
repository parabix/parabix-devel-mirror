...
void PipelineCompiler::computeDataFlowRates(BufferGraph & G) {
    ...
    auto bounded_variable = [&](const BufferRateData & rate) {
        ...
        auto var = free_variable(average(rate));
    ...
    for (auto kernel = firstKernel; kernel <= lastKernel; ++kernel) {
        ...
        for (const auto input : make_iterator_range(in_edges(kernel, G))) {
            ...
            BoundedVars inputRateVar;
            if (LLVM_LIKELY(rate.isFixed())) {
                auto fixedRateVal = maximum(inputRate);
                inputRateVar[LowerBound] = fixedRateVal;
                inputRateVar[UpperBound] = fixedRateVal;
            } else if (LLVM_UNLIKELY(rate.isGreedy())) {
                inputRateVar[LowerBound] = producedRate[LowerBound];
                inputRateVar[UpperBound] = producedRate[UpperBound];
                ++numOfGreedyRates;
            } else {
                inputRateVar = bounded_variable(inputRate);
                Z3_optimize_maximize(ctx, solver, inputRateVar[LowerBound]);
                Z3_optimize_minimize(ctx, solver, inputRateVar[UpperBound]);
            }

            const auto lb = multiply(inputRateVar[UpperBound], stridesPerSegmentVar[LowerBound]);
            const auto ub = multiply(inputRateVar[LowerBound], stridesPerSegmentVar[UpperBound]);
            ...
            if (edge(producerPartitionId, partitionId, H).second) {
                ...
                const auto hasEnough2 = Z3_mk_ge(ctx, producedRate[UpperBound], ub);
                Z3_assert(hasEnough2);
        ...
        for (const auto output : make_iterator_range(out_edges(kernel, G))) {
            ...
            if (LLVM_LIKELY(rate.isFixed())) {
                ...
                auto outputRateVar = bounded_variable(outputRate);
                ...
                VarList[buffer] = { lb, ub };
                Z3_optimize_minimize(ctx, solver, outputRateVar[LowerBound]);
                Z3_optimize_maximize(ctx, solver, outputRateVar[UpperBound]);
            }
        }
    }
    ...
    for (auto kernel = firstKernel; kernel <= lastKernel; ++kernel) {
        ...
        for (const auto input : make_iterator_range(in_edges(kernel, G))) {
            ...
            if (LLVM_LIKELY(rate.isFixed())) {
                ...
                if (edge(KernelPartitionId[producer], partitionId, H).second) {

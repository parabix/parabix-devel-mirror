...
//    };
    ...
    auto minimize_diff =[&](Z3_ast X, Z3_ast Y) {
        Z3_ast args[2] = { X, Y };
        auto r = Z3_mk_sub(ctx, 2, args);
        Z3_assert(Z3_mk_ge(ctx, X, Y));
        auto v = Z3_mk_fresh_const(ctx, nullptr, varType);
        Z3_assert(Z3_mk_eq(ctx, v, r));
        ...
        for (const auto input : make_iterator_range(in_edges(kernel, G))) {
            ...
            if (LLVM_LIKELY(rate.isFixed())) {
                ...
                if (KernelPartitionId[producer] == partitionId) {
                    ...
                    minimize_diff(producedRate[LowerBound], lb);
...
//                const auto expected = average(inputRate);
//                Z3_assert_soft(Z3_mk_gt(ctx, inputRateVar[LowerBound], expected));
//                Z3_assert_soft(Z3_mk_lt(ctx, inputRateVar[UpperBound], expected));
        ...
        for (const auto output : make_iterator_range(out_edges(kernel, G))) {
            ...
            if (LLVM_LIKELY(rate.isFixed())) {
                ...
                // the maximum.
//                const auto expected = average(outputRate);
//                Z3_assert_soft(Z3_mk_lt(ctx, outputRateVar[LowerBound], expected));
//                Z3_assert_soft(Z3_mk_gt(ctx, outputRateVar[UpperBound], expected));
    ...
    for (auto kernel = firstKernel; kernel <= lastKernel; ++kernel) {
        ...
        MinimumNumOfStrides[kernel] = r[LowerBound].numerator();
        assert (r[LowerBound].denominator() == 1);
        MaximumNumOfStrides[kernel] = r[UpperBound].numerator();
        assert (r[UpperBound].denominator() == 1);
    }

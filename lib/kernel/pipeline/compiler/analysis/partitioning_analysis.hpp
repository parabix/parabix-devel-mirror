...
//        nodeRateSet |= inputRateSet;
    ...
    auto checkForPartialSumEntry = [&](const Vertex binding, const Vertex buffer, PartialSumMap & P) -> Vertex {
...
//                K.shrink_to_fit();
    ...
    auto checkFixedRateConversion = [&](const ProgramGraph::vertex_descriptor streamSet, const Vertex buffer) {
        #error here!
    };
    ...
    for (unsigned kernel = 0; kernel < numOfKernels; ++kernel) {
        ...
        for (const auto e : make_iterator_range(in_edges(u, Relationships))) {
            ...
            if (rn.Type == RelationshipNode::IsBinding) {
                ...
                switch (rate.getKind()) {
                    case RateId::Fixed:
                        checkFixedRateConversion(streamSet, buffer);
                        break;
                    case RateId::PartialSum:
                        BEGIN_SCOPED_REGION
                        const auto partialSum = checkForPartialSumEntry(binding, buffer, PI);
        ...
        for (const auto e : make_iterator_range(out_edges(u, Relationships))) {
            ...
            if (rn.Type == RelationshipNode::IsBinding) {
                ...
                switch (rate.getKind()) {
                    ...
                    case RateId::PartialSum:
                        BEGIN_SCOPED_REGION
                        const auto partialSum = checkForPartialSumEntry(binding, buffer, PO);
...
//    orderingOfH.clear();
//    if (LLVM_UNLIKELY(!lexical_ordering(S, orderingOfH))) {
//        report_fatal_error("Cannot lexically order the partition summary graph!");
//    }

//    assert (orderingOfH[0] == 0);
...
//    for (unsigned i = 0; i < PartitionCount; ++i) {
//        reverseMapping[orderingOfH[i]] = i;
//    }

...
bool resolvePropertyDefinition(Name* const property) {
    if (property->hasNamespace()) {
        ...
        if (propit == alias_map.end()) {
            UnicodePropertyExpressionError("Expected a property name but '" +
        ...
        if (isa<BinaryPropertyObject>(getPropertyObject(theprop))) {
            ...
            if (valit != Binary_ns::aliases_only_map.end()) {
                if (valit->second == Binary_ns::N) {
                    Name* binprop = makeName(property_enum_name[theprop],
                                             Name::Type::UnicodeProperty);
        ...
        if ((value == "any") || (value == ".")) {
            ...
            property->setDefinition(
                makeName("blk", "ascii", Name::Type::UnicodeProperty));
            return true;
            ...
            property->setDefinition(
                makeNegativeLookBehindAssertion(makeCC(0, 0x10FFFF)));
            return true;
        } else if (value == "$s") {  // "end anchor ($) in single-line mode"
            property->setDefinition(
                makeNegativeLookAheadAssertion(makeCC(0, 0x10FFFF)));
            return true;
        }
    }
    return false;
...
UnicodeSet resolveUnicodeSet(Name* const name) {
    if (name->getType() == Name::Type::UnicodeProperty) {
        ...
        if (prop.length() > 0) {
            ...
            auto propit = alias_map.find(prop);
            if (propit == alias_map.end()) {
                UnicodePropertyExpressionError(
                    "Expected a property name, but '" + name->getNamespace() +
                    "' found instead");
            }
            auto theprop = static_cast<UCD::property_t>(propit->second);
            auto propObj = getPropertyObject(theprop);
            if ((value.length() > 0) && (value[0] == '/')) {
                assert(false &&
                       "UCD::resolveUnicodeSet(re::Name *) does not support "
                       "regex name resolution, use "
                       "grep::resolveUnicodeSet(re::Name *) instead");
                llvm::report_fatal_error("");
            }
            if ((value.length() > 0) && (value[0] == '@')) {
                // resolve a @property@ or @identity@ expression.
                std::string otherProp =
                    canonicalize_value_name(value.substr(1));
                ...
                if (propit == alias_map.end()) {
                    UnicodePropertyExpressionError(
                        "Expected a property name, but '" + value.substr(1) +
                        "' found instead");
                }
                auto prop2 = static_cast<UCD::property_t>(propit->second);
                auto propObj2 = getPropertyObject(prop2);
                if (isa<BinaryPropertyObject>(propObj) &&
                    isa<BinaryPropertyObject>(propObj2)) {
                    return ~(
                        cast<BinaryPropertyObject>(propObj)->GetCodepointSet(
                            UCD::Binary_ns::Y) ^
                        cast<BinaryPropertyObject>(propObj2)->GetCodepointSet(
                            UCD::Binary_ns::Y));
                } else {
            ...
            const auto& gcobj =
                cast<EnumeratedPropertyObject>(getPropertyObject(gc));
            int valcode = gcobj->GetPropertyValueEnumCode(canon);
            if (valcode >= 0) {
                return gcobj->GetCodepointSet(valcode);
            }
            const auto& scObj =
                cast<EnumeratedPropertyObject>(getPropertyObject(sc));
            ...
            if (propit != alias_map.end()) {
                auto theprop = static_cast<UCD::property_t>(propit->second);
                auto propObj = getPropertyObject(theprop);
                if (BinaryPropertyObject* p =
                        dyn_cast<BinaryPropertyObject>(propObj)) {
                    return p->GetCodepointSet(Binary_ns::Y);
                } else {
                    UnicodePropertyExpressionError(
                        "Error: property " + property_full_name[theprop] +
            ...
            // Now compatibility properties of UTR #18 Annex C
                ...
                return UnicodeSet(0, 0x10FFFF);
            else if (canon == "alnum") {
                Name* digit = makeName("nd", Name::Type::UnicodeProperty);
                Name* alpha =
                    makeName("alphabetic", Name::Type::UnicodeProperty);
                return resolveUnicodeSet(digit) + resolveUnicodeSet(alpha);
            } else if (canon == "xdigit") {
                Name* digit = makeName("nd", Name::Type::UnicodeProperty);
                Name* hexdigit =
                    makeName("hexdigit", Name::Type::UnicodeProperty);
                return resolveUnicodeSet(digit) + resolveUnicodeSet(hexdigit);
            } else if (canon == "blank") {
                Name* space_sep =
                    makeName("space_separator", Name::Type::UnicodeProperty);
                return resolveUnicodeSet(space_sep) +
                       UnicodeSet(0x09) /* tab */;
            } else if (canon == "print") {
                ...
                Name* conn = makeName("connectorpunctuation",
                                      Name::Type::UnicodeProperty);
                Name* join =
                    makeName("joincontrol", Name::Type::UnicodeProperty);
                return resolveUnicodeSet(alnum) + resolveUnicodeSet(mark) +
                       resolveUnicodeSet(conn) + resolveUnicodeSet(join);
            } else if (canon == "graph") {
                ...
                Name* unassigned = makeName("cn", Name::Type::UnicodeProperty);
                return ~(resolveUnicodeSet(space) + resolveUnicodeSet(ctrl) +
                         resolveUnicodeSet(surr) +
                         resolveUnicodeSet(unassigned));
            }
        }
    }
    UnicodePropertyExpressionError(
        "Expected a general category, script or binary property name, but '" +
        name->getName() + "' found instead");
}
